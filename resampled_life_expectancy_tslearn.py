...
data_path = Path("/Users/kylejonespatricia/time_series/WIP/utilities/data/Hierarchical_Amtrak_Ridership.csv")
...
sampled = (
    pd.concat([pivot, clusters], axis=1)
      .groupby("cluster")
      .head(5)
)
...
sax_strings = pd.Series([''.join(seq) for seq in X_sax.reshape(len(X_sax), -1)], index=pivot.index)
